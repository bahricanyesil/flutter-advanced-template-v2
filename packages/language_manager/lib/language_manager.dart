/// Language Manager is a package that provides a way to manage languages
/// for Flutter apps. It allows you to define multiple languages and
/// switch between them easily. It also provides a way to save the
/// language to the database and restore it when the app is opened.
library language_manager;

export 'src/base_slang_language_manager.dart';
export 'src/language_manager.dart';
